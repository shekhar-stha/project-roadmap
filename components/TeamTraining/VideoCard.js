...
export default function VideoCard({ href, thumbnailSrc, title, playlistSize, subData }) {
  return (
    <Link href={href} class="group">
      <div class="relative aspect-h-1 aspect-w-1 w-full max-h-72 overflow-hidden rounded-lg bg-gray-200 xl:aspect-h-8 xl:aspect-w-7">
        <Image
          width={550}
          height={500}
          src={thumbnailSrc}
      ...
      <p class="mt-1 text-sm font-small text-gray-900">{subData}</p>
    </Link>
  );
}
